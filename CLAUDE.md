...
captures the human's voice replies. You can use it to narrate your progress and to ask
the human questions by voice — hands-free.
...
URL). Drive it with plain `curl` — no SDK or keys. These commands are pre-approved in
...
(If the `avatar` CLI is on PATH via `npm link`, `avatar say/state/ask/listen/pipe` also
work and are pre-approved.)
